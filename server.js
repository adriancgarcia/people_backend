...
// Middleware
// cors for preventing cords erros (allows all requests from other origins)
app.use(cors())
// morgan for loggin requests
app.use(morgan("dev"))
...
app.put("/people/:id", async (req, res) => {
    ...
    // send the updfated person as json
        res.json (person);
} catch (error) {
    res.status(400).json({error});
    }
